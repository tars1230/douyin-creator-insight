...
不设置未经基准测试的固定耗时。每次真实运行应记录样本量、Actor、开始/结束时间、失败数和输出大小；不同账号、网络、Actor 队列和上游 schema 下的结果不可直接类推。

## 测试用例

使用 `tests/test_pipeline.py` 中的合成 fixture 作为可重复测试。真实公开账号报告只作为展示样例，不作为性能基准。
